...
**Response**: You could do max - min of both and it would produce
`max(gapminder$year)-min(gapminder$year)=range`, or simply
...
``` r
quantile(gapminder$lifeExp) #Basically producing the max, min and IQR.
```
...
``` r
quantile(gapminder$lifeExp, probs = 0.5) #specifying what percentile you want
```
...
``` r
quantile(gapminder$lifeExp, probs = c(0.25,0.75)) #specifying which probabilities that we want.
```
...
quantile_range <- function(x){
 ...
 quantile_values <- larger_value - smaller_value
